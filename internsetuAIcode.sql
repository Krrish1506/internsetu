-- ===== internsetu_db: cleaned & improved schema =====
...
CREATE TABLE IF NOT EXISTS students (
    ...
    hashed_password VARCHAR(255) NOT NULL,   -- store bcrypt hashes
    cgpa DECIMAL(3,2) NOT NULL,
    college_name VARCHAR(200),
    location VARCHAR(100),
    field VARCHAR(100),                       -- e.g. 'B.Tech', 'MBA', 'BCA'
    skills TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CHECK (CHAR_LENGTH(hashed_password) >= 60) -- bcrypt hashes are ~60 chars; optional depending on MySQL version
);
...
-- INDEXES (drop if exist then create)
DROP INDEX IF EXISTS idx_students_field_location ON students;
CREATE INDEX idx_students_field_location ON students(field, location);

DROP INDEX IF EXISTS idx_internships_field_location ON internships;
CREATE INDEX idx_internships_field_location ON internships(field, location);

DROP INDEX IF EXISTS idx_internships_min_cgpa ON internships;
CREATE INDEX idx_internships_min_cgpa ON internships(min_cgpa);

-- TRIGGER: after inserting a student, create recommendations automatically
-- Note: trigger uses flexible field matching (exact OR substring) and respects Remote/Onsite
...
BEGIN
    ...
    FROM internships i
    WHERE
      -- flexible field matching: exact or substring in either direction (case-insensitive)
      (
      ...
      -- match by location OR allow remote OR if internship has no location specified
      AND (
            i.mode = 'Remote'
...
-- SAMPLE DATA: students (note: hashed_passwords below are placeholders; replace with actual bcrypt hashes from backend)
INSERT INTO students (name, email, hashed_password, cgpa, college_name, location, field, skills)
VALUES
...
-- DATABASE USER (change password & host for your environment)
...
-- EXAMPLE: query to get matching internships for a given student_id (use parameterized queries in backend)
-- Replace :student_id with the ID from your backend parameter (do not concatenate strings in backend)
